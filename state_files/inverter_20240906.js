var currentSerialNum = 1234567890;
...
//				$('#singleParallelNavTabs').fadeIn();
					...
					if(showParallelData) {
						...
						$.each(response.devices, function(index, element) {
							...
							trHtml += '<td>' + (element.lost || (!element.vpv1 && element.vpv1 != 0) ? '' : (element.vpv1 / 10) + 'V') + '</td>';
							trHtml += '<td>' + (element.lost || (!element.ppv1 && element.ppv1 != 0) ? '' : element.ppv1 + 'W') + '</td>';
							trHtml += '<td>' + (element.lost || (!element.vpv2 && element.vpv2 != 0) ? '' : (element.vpv2 / 10) + 'V') + '</td>';
...
//				$('#monitorHolder .batteryImg').click();
			...
			if(response.masterType == 1) {						//Phase 1 System
				...
				if(response._12KUsingGenerator) {
					...
					$('.flowChartHolder[chartTarget=' + snForRefreshInfo + '] .phase1ParallelGridTextHolder .phase1FacText').text(response.genFreq / 100);
				} else {
					$('.flowChartHolder[chartTarget=' + snForRefreshInfo + '] .phase1ParallelGridTextHolder .phase1VacText').text(response.vac / 10);
					$('.flowChartHolder[chartTarget=' + snForRefreshInfo + '] .phase1ParallelGridTextHolder .phase1FacText').text(response.fac / 100);
				}
...
//				$('#monitorHolder .batteryImg').click();
			...
			if(currentDeviceType == 2) {				//AC Charger
				...
				$('.flowChartHolder[chartTarget=' + snForRefreshInfo + '] .pv1PowerText').text(response.ppv1);
				$('.flowChartHolder[chartTarget=' + snForRefreshInfo + '] .pv2PowerText').text(response.ppv2);
				$('.flowChartHolder[chartTarget=' + snForRefreshInfo + '] .pv3PowerText').text(response.ppv3);
				$('.flowChartHolder[chartTarget=' + snForRefreshInfo + '] .vpv1Text').text(response.vpv1 / 10);
				$('.flowChartHolder[chartTarget=' + snForRefreshInfo + '] .vpv2Text').text(response.vpv2 / 10);
				$('.flowChartHolder[chartTarget=' + snForRefreshInfo + '] .vpv3Text').text(response.vpv3 / 10);
			}
			...
			if(isSnaSeries() && response._12KUsingGenerator) {
				...
				if(response._12KUsingGenerator) {
					...
					$('.flowChartHolder .phase3ParallelGridTextHolder .phase3FacsText').text(response.fac / 100);
					$('.flowChartHolder .phase3ParallelGridTextHolder .phase3FactText').text(response.fac / 100);
